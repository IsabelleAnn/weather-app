...
function getBackgroundImgSrc(skies, icon) {
  ...
  if (icon === "50d" || icon === "50n") {
    url = `url(images/${getImageRandomImgName("mist")})`;

    return url;
  }
  if (skies === "Snow") {
    url = `url(images/${getImageRandomImgName("snow")})`;

    return url;
  }
  if (skies === "Thunderstorm" || skies === "Drizzle" || skies === "Rain") {
    url = `url(images/${getImageRandomImgName("rain")})`;

    return url;
  }
  if (skies === "Clear") {
    url = `url(images/${getImageRandomImgName("clear")})`;

    return url;
  }
  if (skies === "Clouds") {
    url = `url(images/${getImageRandomImgName("clouds")})`;
